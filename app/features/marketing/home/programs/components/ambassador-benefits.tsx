...
export function AmbassadorBenefits() {
  ...
  const memoizedAmbassadors = useMemo(() => ambassadors, []);
      ...
      <section className="max-w-6xl mx-auto px-6 py-8 mt-6 text-black dark:text-white relative">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 items-center gap-6 md:gap-12">
          <div className="max-w-md md:max-w-lg text-left pr-0 md:pr-12 z-10">
            <h2 className="text-4xl md:text-5xl font-extrabold mb-3 md:mb-6" style={{ fontFamily: 'var(--font-SF-Pro)' }}>
              {t("Join and be part of the ")}
              <span className="text-pink-500 dark:text-pink-500">{t("Network")}</span>
            </h2>
            ...
            <a href="https://docs.google.com/forms/d/e/1FAIpQLScRo7-7qlgtJqNqj5T7wfS6htJ7LGjKNstdOsaVir1g3Me17w/viewform" target="_blank" rel="noopener noreferrer">
              <Button variant="blackTransparent" className="mt-4 ml-0 md:ml-5 text-black text-md md:text-xl py-2 md:py-4">
                {t("Register")}
              </Button>
            </a>
          </div>
        ...
        <div className="relative mt-6">
          <Carousel className="block md:hidden" onIndexChange={setCurrentIndex}>
            <CarouselContent>
              {memoizedAmbassadors.map((ambassador, index) => (
                <CarouselItem key={index} className="basis-full">
                  <div className="bg-white dark:bg-black/20 p-5 rounded-3xl text-left border border-gray-300 dark:border-white/20 hover:shadow-md hover:bg-gray-200 dark:hover:bg-[#590539] hover:border-pink-600 transition-opacity transition-transform duration-300 will-change-[opacity,transform]">
            ...
            <div className="w-full flex flex-col items-center mt-5 gap-2">
              <div className="flex gap-4">
                <CarouselPrevious className="px-2 py-3 rounded-full border transition-opacity transition-transform duration-300 will-change-[opacity,transform] text-gray-900 dark:text-white border-gray-400 dark:border-gray-600 bg-gray-200 dark:bg-transparent" />
                <p className="text-sm font-bold text-gray-700 dark:text-white mt-2">
                  {currentIndex + 1} / {memoizedAmbassadors.length}
                </p>
                <CarouselNext className="px-2 py-3 rounded-full border transition-opacity transition-transform duration-300 will-change-[opacity,transform] text-gray-900 dark:text-white border-gray-400 dark:border-gray-600 bg-gray-200 dark:bg-transparent" />
              </div>
            </div>
          </Carousel>
          ...
          <div className="hidden md:grid grid-cols-2 gap-6">
            {memoizedAmbassadors.map((ambassador, index) => (
              <div key={index} className="bg-white dark:bg-black/20 p-6 rounded-3xl text-left border border-gray-300 dark:border-white/20 hover:shadow-md hover:bg-gray-200 dark:hover:bg-[#590539] hover:border-pink-600 transition-opacity transition-transform duration-300 will-change-[opacity,transform]">
